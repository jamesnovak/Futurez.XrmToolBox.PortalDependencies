...
{
    public class WebTemplateDependency : PortalConfigDependency
    {
        public WebTemplateDependency(IOrganizationService service, Utility utility, string baseServerUrl) :
        ...
        public override List<DependencyItem> ProcessDependencies(string websiteId, List<Entity> itemsList, bool searchNameOnly = true, string regex = null)
        {
            ...
            regex = "({%-?\\s*(include|extends)?\\s*('|\")?" + SRCH_SLUG + "('|\")?.*-?%})";
